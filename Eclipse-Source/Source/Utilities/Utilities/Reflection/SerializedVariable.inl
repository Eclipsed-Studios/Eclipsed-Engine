...
namespace Eclipse::Reflection
{
	...
	inline void SerializedVariable<T>::DrawElement(U& element)
	{
		...
		if constexpr (std::is_same<U, float>::value) ImGui::DragFloat(GetNameID().c_str(), &element, 0.01f, hasMinMax ? myMin : 0.f, hasMinMax ? myMax : 0.f);
		else if constexpr (std::is_same<U, int>::value) ImGui::DragInt(GetNameID().c_str(), &element, 1.f, hasMinMax ? myMin : 0, hasMinMax ? myMax : 0);
		else if constexpr (std::is_same<U, Math::Vector2<float>>::value) ImGui::DragFloat2(GetNameID().c_str(), &element.x);
