...
#include "EclipsedEngine/Assets/Helper/TextManager.h"
	...
	/*		LOG_ERROR("Failed to load font correctly");
		...
		//for (int fontSize : fontData->setFontSizes)
		{
			...
			for (int i = 0; i < 128; i++)
			{
				...
				_data->myCharTexture.emplace(characterChar, character);
			}

			_data->maxCharHeight = (float)(face->max_advance_height >> 6) * 0.01f;
