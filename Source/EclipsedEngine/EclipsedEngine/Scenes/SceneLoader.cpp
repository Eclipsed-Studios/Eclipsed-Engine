...
namespace Eclipse
{
	void SceneLoader::Save(const Assets::Scene& scene)
	{
		...
		for (auto& [pComp, varList] : list)
		{
			for (auto& var : varList)
			{
				...
				// Keep this in sync with the skip above: components that never
				// overrode GetComponentName() (or otherwise weren't registered
				// in the pass above) have no entry here. Using operator[] in
				// that case would default-construct a null rapidjson::Value,
				// which later crashes/corrupts memory in WriteMember's
				// AddMember call (AddMember requires an object).
				if (compName == "Component")
				{
					continue;
				}
				...
				{
					LOG_ERROR("SceneLoader::Save - no component entry for '%s' (instance id %u), skipping reflected variable '%s'",
						compName.c_str(), pComp->myInstanceComponentID, var->GetName());
					continue;
				}

				auto idIt = compIt->second.find(pComp->myInstanceComponentID);
				if (idIt == compIt->second.end())
				{
					LOG_ERROR("SceneLoader::Save - no matching instance id %u for component '%s', skipping reflected variable '%s'",
						pComp->myInstanceComponentID, compName.c_str(), var->GetName());
					continue;
				}

				WriteMember(idIt->second, var, alloc);
			}
		}
	...
	void SceneLoader::WriteMember(rapidjson::Value& aValue, Reflection::AbstractSerializedVariable* aSerialized, rapidjson::Document::AllocatorType& alloc)
	{
		...
		// Defensive guard: AddMember below requires aValue to already be a
		// JSON object. If a caller ever passes in a default-constructed
		// (null) Value - e.g. from an operator[] lookup that didn't find an
		// existing entry - fail loudly here instead of crashing/corrupting
		// memory inside AddMember.
		if (!aValue.IsObject())
		{
			LOG_ERROR("SceneLoader::WriteMember - target JSON value is not an object (variable '%s'), skipping", aSerialized->GetName());
			return;
		}
