    ...
    void GameSettingsWindow::DrawCollisionLayerEditor()
    {
        ...
        if (ImGui::BeginTable("CollisionLayers", columnCount + 2, tableFlags))
        {
            ...
            for (int i = columnCount - 1; i >= 0; i--)
                ImGui::TableSetupColumn(/*Layer::AsString(Layer(1 << i)).c_str()*/"Layers: Not implemented correctly.", columnFlags);
            ...
            {
                totalID++;
                std::string collisionName = /*Layer::AsString(Layer(1 << i));*/ "Layers: Not implemented correctly.";
                ...
                for (int j = 0; j < columnCount - i; j++)
                {
                    ...
                    if (ImGui::Checkbox("", &hasLayerBool))
                    {
                        ...
                        if (hasLayerBool)
                        {
                            ...
                            PhysicsEngine::myCollisionLayers[i] |= byteShifts;
                        }
                        else
                        {
                            PhysicsEngine::myCollisionLayers[layerCount - 1 - j] &= ~(1 << i);
